...
local window = library:AddWindow("TreeeXMuki | Private Killing - Hello" .. displayName, {
	main_color = Color3.fromRGB(94, 94, 94),
...
local creditsLabel = mainTab:AddLabel("Credits: Empress Treee")
...
local running = false
local thread
local antiFlingSwitch = mainTab:AddSwitch("Anti Fling", function(bool)
	if bool then
...
local hideFramesSwitch = mainTab:AddSwitch("Hide Frames", function(bool)
...
miscellaneousLabel.TextSize = 30
...
local safePlatform
local megaSwitch = killingTab:AddSwitch("Egg + Nan Combo", function(state)
...
killingTab:AddSwitch("Auto Punch With Animation", function(state)
...
local autoPunchNoAnima = killingTab:AddSwitch("Auto Punch Without Animation", function(state)
...
targetKillingLabel.TextSize = 30
...
local specdropdown = killingTab:AddDropdown("Select Player to Spectate", function(text)
...
godmodeTab:AddSwitch("Spawn With Pack DMG", function(enabled)
	wwRespawnEnabled = enabled
end):Set(false)

godmodeTab:AddSwitch("Spawn With Pack HP", function(enabled)
...
packSpamSwitch = godmodeTab:AddSwitch("Pack Spam", function(enabled)
	active = enabled
	if enabled then
		cachePets()
		task.spawn(function()
...
local smartTPHeartbeat = nil
local smartTPCharConn = nil
local selectedSmartTarget = nil
local smartTPTeleportActive = false
local smartTPLastKills = 0
local function smartLockToTarget(char)
	if not selectedSmartTarget or not isPlayerAlive(selectedSmartTarget) then
		return
	end
	local targetChar = selectedSmartTarget.Character
	...
	myHrp.Velocity = Vector3.zero
	myHrp.RotVelocity = Vector3.zero
	if typeof(myHrp.AssemblyLinearVelocity) == "Vector3" then
...
local function startSmartTP()
	if smartTPActive then
		return
	end
	smartTPActive = true
	smartTPTeleportActive = true
	smartTPLastKills = getLeaderstatsKills()
	smartLockToTarget(Player.Character)
	smartTPHeartbeat = RunService.Heartbeat:Connect(function()
		if not smartTPActive then
			return
		end
		if smartTPTeleportActive then
			local currentKills = getLeaderstatsKills()
			if currentKills > smartTPLastKills then
				smartTPTeleportActive = false
				local char = Player.Character
				if char then
					local hum = char:FindFirstChildOfClass("Humanoid")
					if hum then
		...
		if smartTPTeleportActive then
			smartLockToTarget(Player.Character)
			if selectedSmartTarget and isPlayerAlive(selectedSmartTarget) then
				killPlayer(selectedSmartTarget)
			end
		end
	end)
	smartTPCharConn = Player.CharacterAdded:Connect(function(newChar)
		if not smartTPActive then
			return
		end
		smartTPTeleportActive = true
		smartTPLastKills = getLeaderstatsKills()
		smartLockToTarget(newChar)
	end)
end
local function stopSmartTP()
	smartTPActive = false
	smartTPTeleportActive = false
	if smartTPHeartbeat then
		smartTPHeartbeat:Disconnect()
		smartTPHeartbeat = nil
	end
	if smartTPCharConn then
		smartTPCharConn:Disconnect()
		smartTPCharConn = nil
	end
	local char = Player.Character
	if char then
...
local smartTPDropdown = godmodeTab:AddDropdown("Add To Blacklist", function(text)
	for _, plr in ipairs(Players:GetPlayers()) do
		local optionText = plr.DisplayName .. " | " .. plr.Name
		if text == optionText then
			selectedSmartTarget = plr
			break
		end
	end
end)
...
for _, plr in ipairs(Players:GetPlayers()) do
    if plr ~= Player then
        smartTPDropdown:Add(plr.DisplayName .. " | " .. plr.Name)
    end
end
Players.PlayerAdded:Connect(function(plr)
    if plr ~= Player then
...
Players.PlayerRemoving:Connect(function(plr)
    if selectedSmartTarget == plr then
        selectedSmartTarget = nil
    end
end)

local smartTPSwitch = godmodeTab:AddSwitch("Kill Teleport", function(bool)
    if bool then
        startSmartTP()
    else
        stopSmartTP()
